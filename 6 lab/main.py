...
def probabilistic_guarantee(Y, p, t_values):
    ...
    """
    Критерий вероятностной гарантии показывает, 
    с какой вероятностью можно получить прибыль не ниже фиксированного порога t0.
    """
...
def probabilistic_guaranteed_result(Y, p, P_values):
    ...
    """
    Здесь ищем максимальный уровень прибыли, который можно гарантировать с вероятностью не меньше P0.
    """
...
def expected_utility(Y, p, u):
    ...
    """
    Критерий ожидаемой полезности делает то же самое, 
    что и математическое ожидание, но в качестве выигрыша используется не сама прибыль, а её полезность для ЛПР
    """
...
def wald(Y):
    ...
    """
    Критерий Вальда ориентируется на худший исход.
    """
    return Y.min(axis=1)
...
def hurwicz(Y, gamma):
    ...
    """
    Критерий Гурвица — это компромисс между пессимизмом и оптимизмом. Коэффициент γ показывает степень осторожности
    """
    mins = Y.min(axis=1)
    maxs = Y.max(axis=1)
...
def laplace(Y):
    ...
    """
    Если у нас нет информации о вероятностях состояний природы, мы считаем их равновероятными. 
    Тогда оптимальным решением является то, у которого максимальный средний выигрыш
    """
...
def savage(Y):
    ...
    """
    Сожаление — это разница между лучшим возможным результатом в данном состоянии и тем, что получит моя альтернатива
    """
